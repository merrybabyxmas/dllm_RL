"""
Standalone GRPO training script for LLaDA-8B-Instruct.  v2

Changes vs v1:
  - ValueReplayBuffer: (pooled_hidden, reward) pairs for offline value training
  - K-step value update per policy step (--value_k_steps, default 5)
  - Critic quality gate: Stage 2 advantage used only when EMA expvar >= threshold
  - Critic warm-up: first N steps train value head without using it for policy
  - explained_var bug fix: returns nan when target variance is 0
  - Huber loss option for value head (--value_loss_fn huber)
  - Richer Stage 2 logging (rho_std, using_stage2_adv, ema_expvar)
"""
...
class ValueReplayBuffer:
    """
    Circular buffer of (mean_pooled_hidden, reward) pairs for offline
    value head training.  Stores entries on CPU to avoid holding GPU memory.
    """

    def __init__(self, capacity: int = 64) -> None:
        self.capacity = capacity
        self._hiddens: deque = deque(maxlen=capacity)
        self._rewards: deque = deque(maxlen=capacity)

    def push(self, pooled_hidden: torch.Tensor, reward: float) -> None:
        """Add one (hidden, reward) sample.  pooled_hidden: [hidden_size]."""
        self._hiddens.append(pooled_hidden.detach().cpu().float())
        self._rewards.append(float(reward))

    def sample(
        self, batch_size: int, device: torch.device
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """Return (hiddens [B, H], rewards [B]) on device."""
        n = min(batch_size, len(self._hiddens))
        indices = random.sample(range(len(self._hiddens)), n)
        h = torch.stack([self._hiddens[i] for i in indices]).to(device)
        r = torch.tensor([self._rewards[i] for i in indices], device=device)
        return h, r
...
class ValueHead(nn.Module):
    ...
    def forward_pooled(self, pooled: torch.Tensor) -> torch.Tensor:
        """pooled [B, H] (already mean-pooled) → values [B].
        Used by the replay buffer training path."""
        return self.mlp(pooled).squeeze(-1)
...
def compute_policy_loss(
    ...
    # ---- Value diagnostics (no backward here — value training is in main loop) --
    # We compute explained_var from the current rollout's value_estimates for
    # monitoring.  Actual value head gradient updates use the replay buffer.
    value_loss_val = 0.0
    explained_var  = float("nan")
    if cfg.method == "stage2" and value_estimates is not None:
        with torch.no_grad():
            v_pred_diag  = value_estimates.float().detach()  # already computed in rollout
            v_target     = rewards.float().detach()
            value_loss_val = F.mse_loss(v_pred_diag, v_target).item()
            var_target   = v_target.var().item()
            if var_target >= 1e-8:
                res_var      = (v_target - v_pred_diag).var().item()
                explained_var = 1.0 - res_var / var_target
            # else: target variance ≈ 0 (all rewards identical) → expvar undefined → nan
    ...
    with torch.no_grad():
        ...
        if conf_weights is not None:
            ...
            # std of per-token weights (measures how much credit varies across tokens)
            rho_sq   = (conf_weights ** 2 * mask_f).sum() / mask_f.sum().clamp(min=1.0)
            rho_std  = float((rho_sq - mean_conf ** 2).clamp(min=0.0).sqrt())
    ...
    metrics = {
        ...
        "using_stage2_adv":  using_stage2_adv,
    }
...
def parse_args() -> argparse.Namespace:
    ...
    # Stage 2: value head architecture
    p.add_argument("--value_hidden_size", type=int,   default=1024)
    p.add_argument("--critic_lr",         type=float, default=5e-6)

    # Stage 2: value training stability (v2)
    p.add_argument("--value_k_steps",       type=int,   default=5,
                   help="Value head gradient steps per policy step (replay)")
    p.add_argument("--value_replay_size",   type=int,   default=64,
                   help="Replay buffer capacity (number of (hidden,reward) samples)")
    p.add_argument("--value_replay_batch",  type=int,   default=16,
                   help="Mini-batch size drawn from replay buffer each value step")
    p.add_argument("--critic_expvar_gate",  type=float, default=0.05,
                   help="Min EMA explained variance to enable Stage 2 advantage")
    p.add_argument("--critic_warmup_steps", type=int,   default=200,
                   help="Steps to train value head only (no Stage 2 adv for policy)")
    p.add_argument("--value_loss_fn",       type=str,   default="huber",
                   choices=["mse", "huber"],
                   help="Loss function for value head replay training")
...
def main() -> None:
    ...
    ema_expvar: float = 0.0          # EMA of explained variance (gate signal)
    if cfg.method == "stage2":
        ...
        print(f"[init] Replay buffer: capacity={cfg.value_replay_size}, "
              f"batch={cfg.value_replay_batch}, k_steps={cfg.value_k_steps}")
        print(f"[init] Critic gate: warmup={cfg.critic_warmup_steps} steps, "
              f"expvar_gate={cfg.critic_expvar_gate}")
    ...
    for step in range(1, cfg.max_steps + 1):
        ...
        # ----- Value head: push to replay + K-step offline training -----------
        replay_val_loss = 0.0
        if value_head is not None and replay_buffer is not None:
            # Push current rollout's (pooled_hidden, reward) pairs into buffer
            pooled_h = rollout.get("pooled_hidden")   # [G, hidden] or None
            ...
            # K gradient steps from replay (once buffer has enough samples)
            ...
            # Update EMA explained variance (skip nan steps)
            ev = metrics.get("explained_var", float("nan"))
            if not math.isnan(ev):
                ema_expvar = 0.95 * ema_expvar + 0.05 * ev

        metrics["replay_val_loss"] = replay_val_loss
        metrics["ema_expvar"]      = ema_expvar
        ...
        if step % cfg.logging_steps == 0 or step == 1:
            ...
            ev_str = (f"{metrics['explained_var']:.4f}"
                      if not math.isnan(metrics["explained_var"]) else "nan")
            record = {
                "step":              step,
                "loss":              metrics["loss"],
                ...
                "rho_std":           metrics["rho_std"],
                "mean_kl":           metrics["mean_kl"],
                "value_loss":        metrics["value_loss"],
                "replay_val_loss":   metrics["replay_val_loss"],
                "explained_var":     metrics["explained_var"],
                "ema_expvar":        metrics["ema_expvar"],
                "using_stage2_adv":  metrics["using_stage2_adv"],
            ...
            if cfg.method in ("stage1", "stage2"):
                print(f"         conf_mean={metrics['mean_confidence']:.4f}  "
                      f"rho_std={metrics['rho_std']:.4f}")
            if cfg.method == "stage2":
                s2flag = "✓" if metrics["using_stage2_adv"] else "✗(fallback)"
                print(f"         val_loss={metrics['value_loss']:.4f}  "
                      f"replay_vl={metrics['replay_val_loss']:.4f}  "
                      f"expvar={ev_str}  ema_expvar={metrics['ema_expvar']:.4f}  "
                      f"stage2_adv={s2flag}")
